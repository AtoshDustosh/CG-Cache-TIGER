...
class GraphCollator:
    ...
    def collate_memory_nodes(self, nids: np.ndarray, ts: np.ndarray) -> List[Tuple]:
        ...
        for depth in range(len(layers)):
            neigh_nids, neigh_eids, neigh_tss = layers[depth]
    ...
    def _cg_update_daemon(self):
        with torch.cuda.stream(self.cache_stream):
            while True:
                ...
                #     raise RuntimeError(f"Invalid signal ({signal}) for inc update")
...
def compute_delta_std(srcs: np.ndarray, dsts: np.ndarray, ts: np.ndarray) -> float:
    """
